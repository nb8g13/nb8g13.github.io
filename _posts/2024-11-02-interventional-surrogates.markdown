...
Here, $$\phi$$ corresponds to parameters of the intervention map $$\omega$$ whilst $$\psi$$ corresponds to parameters of the surrogate model $$\mathcal{M}^{\psi}$$. If we are careful in our choice of parameter families we can solve the above problem via stochastic gradient descent. With this in mind, let $$q^{\psi, phi}_{\iota}$$ denote the density of the interventional distribution produced by the surrogate $$\mathcal{M}^{\phi}$$ under the intervention $$\omega^{\phi}(\iota)$$. Let us choose both parameter families so that $$q^{\psi, \phi}_{\iota}$$ is tractable and differentiable with respect to $$\phi$$ and $$\psi$$. In this case, we can form Monte Carlo gradient estimates for the abstraction error using ABM outputs $$\mathbf{y}^{(b)}$$ generated by forward-simulating interventions $$\iota^{(b)}$$ sampled from $$\eta$$:
