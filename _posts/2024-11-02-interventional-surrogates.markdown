...
Here, $$\phi$$ corresponds to parameters of the intervention map $$\omega$$ whilst $$\psi$$ corresponds to parameters of the surrogate model $$\mathcal{M}^{\psi}$$. If we are careful in our choice of parameter families we can solve the above problem via stochastic gradient descent. With this in mind, let $$q(\cdot ; \psi, \phi)_{\iota}$$ denote the density of the interventional distribution produced by the surrogate $$\mathcal{M}^{\psi}$$ under the intervention $$\omega^{\phi}(\iota)$$.  By choosing both parameter families so that the densities $$q(\cdot ; \psi, \phi)_{\iota}$$ are tractable and differentiable with respect to $$\phi$$ and $$\psi$$, we can form Monte Carlo gradient estimates for the abstraction error using ABM outputs $$\mathbf{y}^{(b)}$$ generated by forward-simulating interventions $$\iota^{(b)}$$ sampled from $$\eta$$:
...
\nabla_{\phi,\psi}\, d_{\tau,\omega^{\phi}}(\mathcal{M}, \mathcal{M}^{\psi}) 
    ...
    \log
    q(\mathbf{y}_{(b) ; \psi, \phi})_{\iota_{b}}.
$$
