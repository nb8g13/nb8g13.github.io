...
In contrast, $$\omega$$ maps inteverntions in the base model to interventions in the abstract model. Consider, our previous epidemiological example. Our ABM may allow us to perform lockdown interventions to limit disease spread. Since our ODE does not model at the individual level it is unclear what a lockdown intervention should look like at the abstract level. $$\omega$$ is responsible for telling us what intervention in the ODE model corresponds to a lockdown. Unlike, $$\tau$$, it may be difficult for a domain expert to naturally specify a $$\omega$$ map. We will come back to this issue soon!
