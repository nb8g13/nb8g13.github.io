...
Here, $$\phi$$ corresponds to parameters of the intervention map $$\omega$$ whilst $$\psi$$ corresponds to parameters of the surrogate model $$\mathcal{M}^{\psi}$$. In particular, if we choose the surrogate family to be set of differentiable family of simulators, each with tractable density functions $$q_{\psi}$$, then we can optimise the abstraction error via stochastic gradient descent. More specifically we can form Monte Carlo gradient estimates for the abstraction error using ABMs outputs $$\mathbf{y}^{(b)}$$ generated by forward-simulating interventions $$\iota^{(b)}$$ sampled from $$\eta$$:

$$
\nabla_{\phi,\psi}\, d_{\tau,\omega^{\phi}}(\mathcal{M}, \mathcal{M}^{\psi}) 
    \approx \frac{1}{B} \sum_{b=1}^B - \nabla_{\phi,\psi}
    \log
    q_{\psi, \omega_{\phi}(\iota^{(b)})}
    (\mathbf{y}^{(b)}).
$$
