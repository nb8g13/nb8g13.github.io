...
What makes a good abstraction? Say we want to observe a macroscopic output associated with an intervention $$\iota$$ in the ABM. We have two options. We could map $\iota$ to its corresponding abstract intervention $$\omega(\iota)$$ and apply it in the surrogate $$\mathcal{M}^{\prime}$$ to obtain a macroscopic output directly. Contrastingly, we could simulate $$\iota$$ directly via $$\mathcal{M}$$ to obtain a microscopic output and lift it to the macroscopic level via $$\tau$$. Logically, we want both these procedures to produce the same output. In other words, we want the following commutative diagram to hold for any possible base intervention $\iota$!
