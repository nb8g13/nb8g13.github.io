...
class OpinionDynamics:
    ...
    def __init__(self, n_time_Steps=100_000, n_agents=1_000):
...
The details of the model above are not important. SynthPop only requires access to a function which can run the simulator with a given set of parameters. In this case we can run the model via the `init_and_run` method. Before we can use, Synthpop we must first wrap our model in the 
